...
export default class GameController {
  ...
  nextLevel() {
    ...
    if (this.level === 1) {
      ...
      this.enemyTeam = generateTeam(
        Team.getEnemyTeam(),
        2,
        this.userTeam.length + userPositions.length,
      );
      this.addPositionCharacter(this.userTeam, this.enemyTeam);
      ...
      GamePlay.showMessage(
        `Your score ${this.point}. Best score: ${this.maxPoints()}.`,
      );
      return;
    }
  ...
  newGame() {
    ...
    this.currentTheme = themes.prairie;
    this.nextLevel();
    GamePlay.showMessage('Начало новой игры!');
  }
  ...
  onCellEnter(index) {
    ...
    if (!this.blockedBoard) {
      ...
      if (this.selected) {
        ...
        const allowPositions = allowedPositions(
          allowPosition,
          allowDistance,
          boardSize,
        );
        ...
        const allowAttack = allowedPositions(
          allowPosition,
          allowDistanceAttack,
          boardSize,
        );
        ...
        if (this.getIndex(userPositions) !== -1) {
          ...
          && this.getIndex(enemyPositions) !== -1
        ) {
